...
| 業種プリセット | 環境変数ひとつで店名・用語・メニュー・追加入力欄・トップ文言を業種に合わせて切替（汎用テンプレ） |

## 業種プリセット（汎用化）

予約・空き枠計算・通知・管理・LINE 連携の仕組みは業種に依存しません。店名・用語・メニュー・追加入力欄・トップページの文言は **業種プリセット**（[`src/lib/preset.ts`](src/lib/preset.ts)）にまとめてあり、環境変数 `NEXT_PUBLIC_INDUSTRY` を切り替えるだけで業種に合わせられます。

```env
# generic 汎用テンプレ(既定) / auto 自動車整備 / salon 美容室 / clinic 治療院 / dining 飲食
NEXT_PUBLIC_INDUSTRY="salon"
```

| プリセット | 用途 | 追加入力欄の例 |
| --- | --- | --- |
| `generic` | 中立な汎用テンプレ（既定） | なし |
| `auto` | 自動車整備工場 | 車種・ナンバー |
| `salon` | 美容室・サロン | 希望スタイル・指名 |
| `clinic` | 治療院・クリニック | 気になる症状 |
| `dining` | 飲食・予約席 | 人数・席の希望 |

- 追加入力欄は DB カラム（`carModel` / `carPlate`）を汎用の「入力欄1・2」として再利用するため、**スキーマ変更なし**で業種ごとのラベルに変えられます。
- 新しい業種を足すときは `src/lib/preset.ts` にプリセットを 1 つ追加するだけです。
- メニュー初期データもプリセットから投入されるため、`NEXT_PUBLIC_INDUSTRY` を設定してから `npm run db:seed` を実行してください。
